...
|---|---|---|
| `cogny` | a `ref` to [`cogny@4.0.2`](../cogny/README.md) — the reasoning core | every channel of this assistant consults the same second opinion; two cores would be two opinions |
...
and it is addressed as the cell it is. `talky@4.2.0` is a sealed hive and is
...
assistant/
  ...
  cogny/config.json      a ref to cogny@4.0.2
...
`talky@4.2.0`). A parent that sends turns in and does not take the failures back
...
sibling inside the level consumes itself.* Derived from `talky@4.2.0` and
`telegram-connector@2.0.0` in the container, `cogny@4.0.2` and `tools@1.0.0`
...
`in_memory_call` and `in_thread_call` are declared by `talky@4.2.0` and are
...
{"scope": "<member>/assistants", "diff": {
  "add_nodes": [{"name": "agent", "template": "assistant@1.0.0"}]
}}
```
...
{"scope": "<assistant>", "ctx": {"model": "<the brain's model>"}, "diff": {
  "add_nodes": [
    ...
    {"name": "channels/talky", "template": "talky@4.2.0"}
  ],
  "add_edges": [
...
- **No memory leg for the reasoning core.** At this boundary the surface's recall
  ...
  back carries nothing that tells them apart. `cogny@4.0.2` ships `memory_tier`
