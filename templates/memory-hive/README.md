...
| inline-reject | out | `./memory/extract-glue` → your drain | condition `hop.route == 'reject'` — **not optional once the inline ingress is wired.** The two edges belong in the same mutation: a rejected block on an undrained egress is an unrouted dead end, so nobody ever learns the memory was not written. A colony that ran the inline lane for weeks with only `recall-reject` drained is where that lesson comes from |
| recall-reject | out | `./memory/recall` → your drain | condition `hop.route == 'reject'` — a HALF window (exactly one of `recall_window_from`/`_to` non-empty) is a caller bug and leaves here at request entry, before the leg fan. **Drain it**: unrouted it is a dead end, and the caller waits for a bundle that never comes |
