...
The occupant of this level is `member@1.0.0`. Its accepts list is `in_turn`,
...
is a **member** with its own name, instantiated from `member@1.0.0` like any other. It
...
1. **The organisation.** One `add_nodes` with the template `org@1.0.0`, plus the transit
...
2. **Each member, afterwards, one at a time.** An `add_nodes` into `<org>/members` with
   `member@1.0.0`, and in the *same* mutation the edges that member needs. The container
