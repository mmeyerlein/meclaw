...
| every exit | `hop.turn_id` is on `pass`, `reject` and `hold` alike — from the screen and from `./warden`, the second producer of the first two — and it is not this hive's to mint: the channel assigned it when it accepted the turn (display-hive § 8.2) and the answer owes it back, so the hive carries it through and leaves it empty for a channel that stamps none. A refused turn is still a turn and still has its id, and a turn a person releases re-enters under the id it was spoken under: the warden stores it with the parked row and reads it back off that row, because the release lane carries a hold id and a decision and nothing else. The one deliberate blank is the expiry receipt — the pile read is bounded to five columns and does not carry the parked context. Since `firewall@2.3.1` — before it, every verdict rebuilt the header from a fixed key set the id was not in, and the answer the member finally read named a different turn than the one they spoke. |
