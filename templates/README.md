...
| [`display`](display/) | 2.2.3 | One screen, reached under its own mount on the colony's listener, that many agents and applications write onto at the same time, where a view is a named, owned, optionally expiring piece of that screen, a hold-to-talk button speaks to a mounted `voice` cell over the page's own socket, the screen brings its own design language and a catalogue of twenty-six components an application may name without defining them, and it curates what it shows: a focus number, a relevance per window, a due clock and a judge. |
