...
| [`assistant`](assistant/) | 2.7.0 | One generation of one person's agent as a composition level, holding the reasoning core, the tool surface and one conversation surface per channel that asks for its own, as four refs and no container. |
| [`browser`](browser/) | 1.0.0 | A web page on a screen, as a cell: one browser per member, a context per identity, a page per card, and each page's picture streamed onto a topic of the display's own socket, with pointers and keys coming back the same way. The browser itself is a prerequisite out of the machine's package system, and one whose own sandbox does not hold does not start. |
| [`builder`](builder/) | 1.11.0 | The intake that turns a structural wish into a manifest somebody else submits, on a fast lane of parameterised recipes and a design lane that consults `builder-librarian`. Its `grow_level` recipe renders the transit edges a new level gets from its parent, and the doors of a container level carry `context.org` or `context.member` beside the lane, so a second member of one organisation is a second address. A grown screen hears its channels' failures as system notices, and takes a view down as readily as it puts one up. |
...
| [`collector`](collector/) | 4.1.1 | Context assembly in a hive of two cells: it decides in one place what enters an agent's context window and what leaves it, and folds each tool round back in. |
| [`colony-view`](colony-view/) | 1.1.2 | One app that draws the colony: a committed mutation triggers a topology snapshot, a `code` cell turns it into one component tree, and the view leaves the hive towards a display. |
...
| [`display`](display/) | 2.6.0 | One screen, reached under its own mount on the colony's listener, that many agents and applications write onto at the same time, where a view is a named, owned, optionally expiring piece of that screen; one canvas shows what is in focus, a dock of tiles of one size shows everything present, and an OS mark at the bottom right is the hold-to-talk button that speaks to a mounted `voice` cell over the page's own socket. The screen brings its own design language and a catalogue of thirty components an application may name without defining them, and curates what it shows: a relevance per window, a bar, a due clock and a judge. There is one state per member, held as one row in the store and computed by a pass that is byte-identical with the reference model of the one document describing this template; every output renders that state by its own profile, and `/<mount>/` is the switch that leads to one of them. |
...
| [`freeswitch`](freeswitch/) | 2.0.6 | A telephone as one channel of a person, in two halves inside one hive: a `voice` cell carries the media on its mount and a small state machine of a `code` cell keeps the calls. |
| [`meclaw-os`](meclaw-os/) | 1.8.10 | The colony shell: the outermost of the four composition levels, four occupants, one empty container and the transit graph between them, and no cell of its own. |
| [`member`](member/) | 1.8.0 | One person as a composition level, holding the memory, the curated record, the screen and the keys, plus three open containers for that person's assistants, channels and apps. An app of theirs may put a view up and take it down again. |
...
| [`voice`](voice/) | 2.0.4 | A spoken conversation behind one cell, one WebSocket surface and one pair of provider credentials, where the connection is the session: opening the socket mints an id and closing it ends the session. |
