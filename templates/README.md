...
| [`canvy`](canvy/) | 2.3.1 | One interactive canvas of the colony, reached under its own mount on the listener. Deprecated since [#455](https://github.com/mmeyerlein/meclaw/issues/455), which withdraws it: a screen belongs to a person and a view of the colony is one application among many, so use `display` and `colony-view`. |
...
| [`web`](web/) | 2.0.4 | A display in one cell, reached at `/<mount>/` on the colony's one listener, with a token stylesheet shipped as seed data, so a colony opens a second display by instantiating this template again under another mount. |
