...
| [`assistant`](assistant/) | 2.8.1 | One generation of one person's agent as a composition level, holding the reasoning core, the tool surface and one conversation surface per channel that asks for its own, as four refs and no container. |
...
| [`builder`](builder/) | 1.12.2 | The intake that turns a structural wish into a manifest somebody else submits, on a fast lane of parameterised recipes and a design lane that consults `builder-librarian`. Its `grow_level` recipe renders the transit edges a new level gets from its parent, and the doors of a container level carry `context.org` or `context.member` beside the lane, so a second member of one organisation is a second address. A grown screen hears its channels' failures as system notices, and takes a view down as readily as it puts one up. Its `install_app` recipe installs an application from the declaration the app's template carries under `app`. |
| [`builder-librarian`](builder-librarian/) | 2.2.1 | Lexical retrieval over the builder's knowledge base in one sealed hive: an FTS5 corpus, a BM25 retriever and a catalogue cell, with no embeddings and no model. A catalogue row carries `CONTRACT --`, `STORES --` and `PARAMS --`, the three demands the mutation door enforces. |
...
| [`cogny`](cogny/) | 5.0.3 | One hive holding the agent core, a `collector`, a `dispatcher`, one `llm` brain and a `schemas` cell, for one class of question: synthesis, multi-step work, research. |
| [`collector`](collector/) | 4.2.1 | Context assembly in a hive of two cells: it decides in one place what enters an agent's context window and what leaves it, and folds each tool round back in. |
...
| [`meclaw-os`](meclaw-os/) | 1.8.13 | The colony shell: the outermost of the four composition levels, four occupants, one empty container and the transit graph between them, and no cell of its own. |
| [`member`](member/) | 1.9.3 | One person as a composition level, holding the memory, the curated record, the screen and the keys, plus three open containers for that person's assistants, channels and apps. An app of theirs may put a view up and take it down again. |
...
| [`talky`](talky/) | 5.2.2 | One template holding a whole conversational agent: `session-keeper`, `collector` and `dispatcher` as referenced units under one hive, plus an `llm` brain, the sidecar splitter, a `schemas` cell declaring the sections its own model is asked for, and one error collector. |
...
| [`tools`](tools/) | 1.4.3 | The tool surface of one assistant, one node with one contract, `tool_call` in and `tool_result` out, that also hands back the schemas of the tools it serves. |
