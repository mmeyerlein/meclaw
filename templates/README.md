...
| [`cogny`](cogny/) | 3.0.11 | The agent core as one node: a tool loop of `collector` + `dispatcher` around an llm brain slot, every internal edge pre-wired. A talky without a channel. Asked on `in_turn`; answers on `answer`, calls tools on `tool`, asks a memory on `recall` and drains a failed inference on `error`. |
| [`collector`](collector/) | 2.1.2 | Context assembly as a hive: a rolling conversation window, the memory bundle and the tool round fanned back in -- each leg capped by configuration, not by a model's judgement. |
...
| [`talky`](talky/) | 3.0.14 | The full composite agent: `session-keeper`, `collector`, `dispatcher` and `summarizer` around an llm brain slot, with the loopback, the close path and the handover return already wired. |
