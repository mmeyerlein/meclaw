...
| [`builder`](builder/) | 1.10.0 | The intake that turns a structural wish into a manifest somebody else submits, on a fast lane of parameterised recipes and a design lane that consults `builder-librarian`. Its `grow_level` recipe renders the transit edges a new level gets from its parent, and the doors of a container level carry `context.org` or `context.member` beside the lane, so a second member of one organisation is a second address. A grown screen hears its channels' failures as system notices. |
...
| [`display`](display/) | 2.3.3 | One screen, reached under its own mount on the colony's listener, that many agents and applications write onto at the same time, where a view is a named, owned, optionally expiring piece of that screen; one canvas shows what is in focus, a dock of tiles of one size shows everything present, and an OS mark at the bottom right is the hold-to-talk button that speaks to a mounted `voice` cell over the page's own socket. The screen brings its own design language and a catalogue of twenty-eight components an application may name without defining them, knows which display it is shown on, and curates what it shows: a focus number, a relevance per window, a due clock and a judge. |
...
| [`freeswitch`](freeswitch/) | 2.0.4 | A telephone as one channel of a person, in two halves inside one hive: a `voice` cell carries the media on its mount and a small state machine of a `code` cell keeps the calls. |
...
| [`voice`](voice/) | 2.0.2 | A spoken conversation behind one cell, one WebSocket surface and one pair of provider credentials, where the connection is the session: opening the socket mints an id and closing it ends the session. |
