...
| [`meclaw-os`](meclaw-os/) | 1.0.1 | The colony shell, as the outermost composition level: the capability broker (`access`) and the control loop (`steward`) every organisation shares, plus one open, empty container the organisations are instantiated into. Seven lanes in, nine out, nineteen edges, and no cell of its own. |
...
| [`steward`](steward/) | 2.0.11 | The colony's control loop: charter, deterministic measurement, a judge that simulates before it decides, a params update to the cell it names, an immediate health check, and keep-or-revert after the window -- every cycle a receipt. Ships with every goal disabled. |
