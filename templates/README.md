...
| [`memory-hive`](memory-hive/) | 2.0.1 | Agent memory as a hive: an LLM-free write path, a token-budgeted tier-0 bundle, a four-leg retrieval fan fused without a model, and a nightly consolidation that supersedes instead of deleting. Ten cells, no Rust. |
