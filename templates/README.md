...
| [`meclaw-os`](meclaw-os/) | 1.0.3 | The colony shell, as the outermost composition level: the capability broker (`access`) and the control loop (`steward`) every organisation shares, plus one open, empty container the organisations are instantiated into. Seven lanes in, nine out, nineteen edges, and no cell of its own. |
| [`member`](member/) | 1.0.2 | One person, as a composition level: the memory (`memory-hive`), the curated record (`affinity`) and the screen (`firewall`) every assistant of that person shares, plus one open, empty container the assistants are instantiated into. Four lanes in, seven out, eighteen edges. Two assistants of one person must know the same person and must meet one attacker, which is why all three stand here and not one level down (GH #122). |
...
| [`org`](org/) | 1.0.2 | An organisation as a composition level, and as nothing more: a name, a boundary and one open, empty container its members are instantiated into. No cell at all -- four transit lanes in, seven out, eleven edges. A level that shares nothing is still a level when what it is worth is the namespace. |
