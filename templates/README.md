...
| [`meclaw-os`](meclaw-os/) | 1.0.4 | The colony shell, as the outermost composition level: the capability broker (`access`) and the control loop (`steward`) every organisation shares, plus one open, empty container the organisations are instantiated into. Seven lanes in, nine out, nineteen edges, and no cell of its own. |
...
| [`vault`](vault/) | 1.1.0 | A secret store with no operation that returns a secret -- not a policy over a store, but a cell type whose route surface has no read on it. Secrets enter over the user channel only; the broker may ask it to USE one, or to DELIVER one sealed. |
