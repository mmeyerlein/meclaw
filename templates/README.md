...
| [`canvy`](canvy/) | 0.3.2 | An interactive canvas the colony serves itself over HTTP: a `code` cell draws the picture server-side, the browser owns only the drag. The first thing it draws is the colony. Ask it for a fresh snapshot on `in_refresh`; the drawn page leaves on `surface`. |
