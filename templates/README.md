...
| [`freeswitch`](freeswitch/) | 1.1.0 | A telephone becomes one channel of a person, in two halves inside one hive. The media half is a `voice` cell that binds a WebSocket port and puts ordinary text turns on the topology; the signalling half is a small state machine of a `code` cell that offers `call` and `hangup`, one that keeps the book, a `web_fetch` to the switch and a store of the calls. They live in one hive because one id binds both halves: the channel UUID of the switch is also the session an answer is spoken back into. FreeSWITCH stays the media edge in gateway mode; nothing here speaks SIP. A second call is decided rather than ignored: `second_call` is `busy`, `queue` or `parallel` up to a declared capacity, and every outcome leaves a row in the call table and a receipt on its own lane. It succeeds `phone@1.0.0`, a name that no longer exists, and the migration is in its own README. |
...
| [`voice`](voice/) | 1.4.0 | A spoken conversation behind one cell: one WebSocket surface, one pair of provider credentials, one wire up and one wire down. It has no persona, no memory and no answer of its own, and it carries turns between somebody talking and whatever you put behind it. The connection is the session: a client opens the socket, the cell mints an id, and closing the socket ends it, so there is nothing to resume and nothing to collect. Every emission names that id twice, as `session_id` and as `call_id` — the second is what a channel addresses an answer by, because the first has a second owner one level up. Providers are chosen per instance, and the recogniser takes a list of keyterms for names a general model has never heard. |
