...
CLIENT_CSS = r"""/* colony-view's own look.
   ...
   A `:hover` rule in here asks the screen's root first. `display-hive.md`
   § 6.4: "without `pointer` and without `touch` no `:hover` rules" -- a
   television is an output device, its profile carries `inputs: []`, and a
   hover state on it is a promise nothing can keep
   ([#715](https://github.com/mmeyerlein/meclaw/issues/715)). The gate is the
   screen's own word, `data-inputs` on `.display-columns`, so this sheet needs
   no second sheet and no media query. */
...
/* The view's own size, inside whatever slot a screen gives it. Nothing here
   reaches the document: `html`, `body` and `:root` belong to the screen, and a
   view that wrote them would paint over every other view beside it (#671). */
.colony-view{ position:relative; height:62vh; min-height:22rem; background:var(--bg); color:var(--ink);
...
/* The detail panel. Fixed to the right edge rather than a flex column, because
   the canvas is one positioned element inside a view slot and the SVG has to
   keep the whole width to scale into. */
...
.display-columns:is([data-inputs~="pointer"], [data-inputs~="touch"])
  .colony-view .detail .rel:hover{ color:var(--accent);
...
.display-columns:is([data-inputs~="pointer"], [data-inputs~="touch"])
  .colony-view .legend .unwired-toggle:hover{ border-color:var(--accent);
  color:var(--accent); }
