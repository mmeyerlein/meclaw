...
`grow-canvy.json` adds [`canvy@2.3.2`](../../templates/canvy/) — a timer, two `code` cells and a
...
[`meclaw-os@1.8.12`](../../templates/meclaw-os/) ships, and the reason a shell is the thing you
...
├── colony.json            substrate defaults. two lines.
├── main/config.json       type: "hive", ONE edge, and not one cell
└── main/os/config.json    {"cell": {"type": "ref", "template": "meclaw-os@1.8.12"}}
```
...
address is [`meclaw-os@1.8.12`](../../templates/meclaw-os/). A second boot finds nothing to grow.
