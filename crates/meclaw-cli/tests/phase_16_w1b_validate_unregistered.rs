...
//! a WARNING (exit 0, nginx -t role), `--validate-strict` promotes it to a hard error
...
/// `--validate --validate-strict` on the same state: the unknown cell dir is a hard error.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn validate_strict_fails_on_unregistered_node() {
    ...
    assert!(
        res.is_err(),
        "--validate --validate-strict must FAIL on an unregistered cell dir at reboot"
    );
}
