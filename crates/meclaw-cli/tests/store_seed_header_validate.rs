...
//! (`{"schema":{…}}`) used to pass `meclaw --validate --validate-strict` with exit 0:
...
/// Same reproducer under `--validate-strict` — the builder's G1 gate.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
...
"#,
    ...
    assert!(
        ...
        "--validate --validate-strict must fail on a headerless seed file"
    );
}
...
async fn valid_seed_passes_validate_strict() {
    ...
    run(cli_validate(td.path(), true))
        .await
        .expect("a well-formed seed file must keep --validate --validate-strict green");
}
...
async fn absent_seed_passes_validate_strict() {
    ...
    run(cli_validate(td.path(), true))
        .await
        .expect("an absent seed file must keep --validate --validate-strict green");
}
