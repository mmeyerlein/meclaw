...
/// **e1** — `--validate` (also `--validate-strict`) is clean on a config-only copy and
...
async fn e1_validate_passes_on_a_config_only_copy_without_side_effects() {
    ...
    run(cli_validate(&restored, true))
        .await
        .expect("--validate --validate-strict must pass on a clean config-only restore");
    assert!(
        !restored.join("main/tick/cell.db").exists(),
...
async fn e2_validate_passes_on_a_full_copy_as_a_reboot() {
    ...
    run(cli_validate(&restored, true))
        .await
        .expect("--validate --validate-strict must pass on a full restore");
}

/// **e3** — `--validate --validate-strict` catches the half restore: a cell directory
...
async fn e3_validate_strict_flags_a_restored_cell_dir_the_registry_does_not_know() {
    ...
    assert!(
        ...
        "--validate --validate-strict must FAIL: a restored dir the registry does not know \
