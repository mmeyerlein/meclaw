...
//! `ColonyMsg::ReadTemplates` analogously to the 12-B JSON handler. Since the
//! W13 hardening the `type` filter takes effect — the form had been sending the
//! param into a handler that dropped it.
...
pub struct TemplatesUiQuery {
    ...
    /// Cell-type filter: exact match on the template's `config.json` type.
    #[serde(rename = "type")]
