//! The steward, grown into the reference colony (GitHub #155).
//!
//! Two things this file establishes, and the second one is the interesting one:
//!
//! 1. The steward is an ordinary declaration. `grow-steward.json` adds seven
...
//!
//! That is a stronger property than any rule inside the steward. A control loop
...
async fn the_steward_grows_as_an_ordinary_declaration() {
    ...
    assert!(
        committed(&outcome),
        "the steward is an ordinary declaration: {outcome:?}"
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn the_steward_cannot_wire_its_own_mutation_lane() {
    ...
    // by name — at any scope, from any cell, including from the steward's own
...
async fn a_grown_steward_is_silent_until_a_goal_is_enabled() {
    ...
    // it boots, it grows, and it emits nothing at all.
    let Some(_) = shipped_steward() else {
        return;
    };
    let td = tree();
    ...
    assert!(
        dl.is_empty(),
        "a grown steward dead-letters nothing: {dl:?}"
    );
