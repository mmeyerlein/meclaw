...
//!    and once against one carrying `web@2.0.1`'s seeded demo, which is what
...
fn the_display_is_a_reference_with_its_mount_overridden() {
    ...
    assert_eq!(web["cell"]["template"], "web@2.0.1");
...
/// (GH #402).
///
/// `canvy/web` is a `ref` to `web@2.0.1`, and that template seeds a demo:
...
fn a_display_holding_a_foreign_page_is_bootstrapped_too() {
    ...
    // The shipped `web@2.0.1` seed, in the shape `query` answers with: a page
...
/// real `canvy/web` gets — the ref brings `web@2.0.1`'s own demo seed with it.
...
/// what `canvy/web` is: it is a `ref` to `web@2.0.1`, whose `seed/` ships a demo
