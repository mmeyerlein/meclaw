...
const README: &str = "../../templates/steward/README.md";

/// The steward's shipped page, as bytes.
///
...
fn readme() -> String {
    std::fs::read_to_string(README).expect("the steward ships a README")
}
...
/// answer itself travels in `st_carry` (the `./meter -> ./receipts` edge puts
/// `hop.carry` there).
fn resume(
    kind: &str,
    ...
    emit(
        ...
        serde_json::json!({
            "messages": [{"origin":"tool","type":"tool_result","id":"c1",
                          "text": rows.to_string()}],
            "header": {"hop": {"operation": "select"},
                       "context": {"st_phase": "wait", "st_carry": ledger.to_string()}}
        }),
    )
}
...
fn the_readme_carve_out_is_the_pair_the_edge_test_holds() {
    ...
    assert!(
        doc.contains("`STEWARD_COLONY_DB` is gone"),
        "a deleted setting is retracted on the page, never quietly dropped"
    );
    for path in [METER, PROBE] {
        let s = script(path);
        assert!(
            !s.contains("STEWARD_COLONY_DB"),
...
/// steward's `/colony/ledger` edges follow a precedent rather than set one.
///
