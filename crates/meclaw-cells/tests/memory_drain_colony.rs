...
//! out of `builder/templates/memory-hive/` (GH #125). The memory hive is
...
fn build_tree(td: &tempfile::TempDir) {
    ...
    copy_cells(
        &repo("builder/templates/memory-drain"),
        &root.join("main/drain"),
    );
...
async fn the_same_batch_twice_moves_no_row() {
    ...
    grown.push(("user", "and my sons?"));
    grown.push(("assistant", "leroy and elias"));
    ...
    assert_eq!(rows[6].content, "and my sons?");
    assert_eq!(rows[7].content, "leroy and elias");
    assert_eq!(dlq_count(td.path()), 0, "nothing dead-letters on the way");
}
