...
/// W13 hardening, orchestrator ruling on GH #46 point 3: when a repeated
/// `task_id` arrives WHILE a task is running, both rejections apply and
/// **`harness_busy` wins**.
///
/// The order is not an accident of the code, it is the only order the check can
/// have: the dedup verdict comes out of the tombstone INSERT, and the tombstone
/// insert is the same statement that claims the slot. Reversing the two would
/// mean probing the register before deciding whether the cell can take work at
/// all — a second read on every start, to change nothing but a label.
///
/// The rejection is correct either way; only the reported code was ever in
/// question. Pinning it here makes today's answer the answer, so #46 point 3
/// closes as "decided" rather than "open", and a future flip is a deliberate
/// change to this test rather than a silent drift in a caller's error handling.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn busy_beats_dedup_when_both_apply() {
    let mut r = rig(json!({}));
    r.call("start_task", start_args("t-1")).await;
    let _ = r.reply();
    let _ = r.reconfig_rx.try_recv();

    // Same task_id, first run still going: busy AND already-used both hold.
    r.call("start_task", start_args("t-1")).await;
    let reply = r.reply();
    assert_eq!(
        reply["header"]["error_code"], "harness_busy",
        "occupancy is checked first, so it is the code that is reported"
    );

    // The contrast that gives the pin its meaning: the very same payload
    // answers invalid_input once the harness is free again (the case already
    // covered by `a_repeated_task_id_is_refused_even_after_the_first_finished`).
    r.event(frame(
        json!({"type": "result", "is_error": false, "result": "done"}),
    ))
    .await;
    ...
    assert_eq!(
        r.reply()["header"]["error_code"],
        "invalid_input",
        "free harness, same id: the dedup verdict is the one left standing"
    );
}
