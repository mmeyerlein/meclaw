...
//!
//! - stdin carries the body slots, BOTH header compartments and the envelope
//!   fields — and never `params` (configuration travels via `${VAR}` only);
...
async fn stdin_carries_body_headers_and_envelope_but_never_params() {
...
sys.stdout.write(json.dumps({"messages": [], "echo": d}))
"#;
    let ems = run(
        script,
        false,
    ...
    // Body slots travel.
    assert_eq!(echo["messages"][0]["text"], "hi");
    assert_eq!(echo["custom_slot"]["a"], 1);
    // Both header compartments travel.
    assert_eq!(echo["header"]["context"]["iter"], "3");
    assert_eq!(echo["header"]["context"]["session_id"], "s1");
    assert!(echo["header"]["hop"].is_object());
    // Envelope fields travel.
    assert_eq!(echo["target"], "/code");
    assert_eq!(echo["reply_to"], "/sink");
    assert!(echo["trace_id"].is_string());
    assert_eq!(echo["ttl"], 64);
    // params NEVER travel — configuration is ${VAR} substitution only.
    assert!(
        echo.get("params").is_none(),
        "the script must not see its params: {echo}"
    );
}
