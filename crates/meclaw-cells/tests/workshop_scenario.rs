...
async fn boot_workshop() -> Workshop {
    ...
    // `${VAR}` config for the brain + the collector knobs. The workshop turn
    // takes ten brain entries, so the seam cap gets headroom; the round slate
    // must carry every result of the turn for the final report.
    std::fs::write(
        root.join(".env"),
        format!(
            "WORKSHOP_WS={}\nFETCH_URL=http://{}/doc\nMCP_CMD={}\nHARNESS_CMD={}\n\
             COLLECTOR_MAX_ITER=16\nCOLLECTOR_ROUND_BYTES=200000\nCOLLECTOR_TOOL_CHARS=8000\n",
            ws.display(),
            fetch_addr,
            MCP_FIXTURE,
            HARNESS_FIXTURE
