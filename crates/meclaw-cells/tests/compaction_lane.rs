...
async fn the_store_still_holds_every_folded_round_after_the_compaction() {
    ...
    // Within one round the assistant turn and the tool result reach the
    // collector over two different paths (dispatcher -> collector directly vs.
    // dispatcher -> tool-a -> collector), so their ARRIVAL order -- and with it
    // the rowid order -- is not guaranteed (GH #129). The claim here is
    // completeness, not physical order: compare under the same total order the
    // collector itself uses at read time (iter, then user < assistant < tool).
    fn rank(role: &str) -> i64 {
        match role {
            "user" => 0,
            "assistant" => 1,
            "tool" => 2,
    ...
    let mut roles: Vec<(i64, &str)> = rows.iter().map(|(i, r, _)| (*i, r.as_str())).collect();
    roles.sort_by_key(|(i, r)| (*i, rank(r)));
    assert_eq!(
        roles,
        vec![
            (0, "user"),
