...
//!
//! `templates/steward/clock` shipped a `params.schedules[0]` the `timer` cell
...
//!
//! `meclaw-os@1.0.0` refs the steward, so this reached every tree grown from the
...
//! test that grew the steward stopped at the mutation, which is exactly where
...
async fn a_colony_that_grew_a_steward_starts_again() {
    ...
    if let Err(errors) = second {
        panic!(
            "a colony that grew a steward refused to start again — the defect \
