//! `voice@2.0.2` — the template, its declared surface, and the binding manifest
...
fn the_template_declares_a_long_running_voice_cell() {
    ...
    assert_eq!(tpl["version"], json!("2.0.2"));
...
fn the_readme_manifest_binds_the_three_lanes_and_the_way_back() {
    ...
    assert_eq!(nodes[0]["template"], json!("voice@2.0.2"));
...
async fn the_readme_manifest_grows_the_channel_it_describes() {
    ...
    ack_rx
        .await
        .expect("rescan acked")
        .expect("the library must register voice@2.0.2");
