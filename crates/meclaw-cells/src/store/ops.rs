...
/// The complete set of top-level argument keys one operation reads, or `None`
/// for an operation this dispatcher does not know.
///
/// One list per operation, and it is the SAME list the op then reads — every
/// entry here has a matching `args.get(...)` in the function below, and nothing
/// the ops read is missing. Keeping the two in step is the whole point: a key
/// that is not on the list is a key the store would have ignored.
fn known_args(op: &str) -> Option<&'static [&'static str]> {
    Some(match op {
        "insert" => &["operation", "table", "row"],
        "select" => &[
            "operation",
            "table",
            "columns",
        ...
        "update" => &["operation", "table", "set", "where"],
        "delete" => &["operation", "table", "where"],
        "create_table" => &["operation", "table", "columns"],
        "search" => &[
            "operation",
            "table",
            "match",
        ...
        "canonicalize" => &["operation", "table", "column"],
        "alias_candidates" => &[
            "operation",
            "table",
            "column",
...
/// Reject a payload that carries a top-level key the named operation does not
/// read (W13 hardening, orchestrator ruling).
///
/// The motivating case is not exotic, it is a typo: `{"operation":"delete",
/// "table":"facts","wher":{…}}` used to render `DELETE FROM facts` — no `WHERE`,
/// every row gone, `rows_affected` reporting the damage as success. The store is
/// already this strict one level down (`where.<col>` rejects an unknown operator
/// rather than falling back to equality, `order_by.dir` rejects anything outside
/// `asc|desc`); a silently ignored key at the TOP level was the one place where
/// a misspelling still bought a different statement instead of an error.
///
/// Unknown OPERATIONS are left to the dispatcher's own arm, so `unknown op` keeps
/// its wording.
fn reject_unknown_args(
    op: &str,
    args: &meclaw_core::serde_json::Map<String, Value>,
) -> Result<(), String> {
    let Some(known) = known_args(op) else {
        return Ok(());
    };
    for key in args.keys() {
        if !known.contains(&key.as_str()) {
            return Err(format!(
                "{op}: unknown argument {key:?} (known: {})",
                known.join(", ")
            ));
        }
    }
...
mod tests {
    ...
    fn a_fuzzy_neighbour_is_never_merged_automatically() {
        let conn = two_dimension_fixture();
        mint_subject(&conn, "f1", "Alvaro");
        mint_subject(&conn, "f2", "Alvin");
    ...
    /// W13 hardening, orchestrator ruling: a misspelled top-level key is a
    /// reject, not a different statement.
    ///
    /// The `delete` case is the one with teeth — before the ruling this payload
    /// rendered `DELETE FROM t` and reported the wipe as `rows_affected: 2`.
    #[test]
    fn a_misspelled_where_does_not_delete_the_table() {
        let conn = rusqlite::Connection::open_in_memory().unwrap();
        conn.execute("CREATE TABLE t (a INTEGER)", []).unwrap();
        conn.execute("INSERT INTO t VALUES (1),(2)", []).unwrap();
        let err = dispatch(
            &conn,
            &json!({"operation":"delete","table":"t","wher":{"a":1}}),
        )
        .expect_err("an unknown top-level key is invalid_input, not an outcome");
        assert!(err.contains("unknown argument \"wher\""), "got: {err}");
        let left: i64 = conn
            .query_row("SELECT count(*) FROM t", [], |r| r.get(0))
            .unwrap();
        assert_eq!(left, 2, "nothing was deleted");
    }

    /// The strictness is per-operation, not one flat list: a key that is
    /// legitimate for one op is still a reject on an op that does not read it.
    #[test]
    fn unknown_top_level_args_are_rejected_per_operation() {
        let conn = rusqlite::Connection::open_in_memory().unwrap();
        conn.execute("CREATE TABLE t (a INTEGER)", []).unwrap();
        for (payload, wanted) in [
            (
                json!({"operation":"select","table":"t","columns":["a"],"limt":1}),
                "limt",
            ),
            (
                json!({"operation":"insert","table":"t","row":{"a":1},"where":{"a":1}}),
                "where",
            ),
            (
                json!({"operation":"update","table":"t","set":{"a":1},"order_by":[]}),
                "order_by",
            ),
        ] {
            let err = dispatch(&conn, &payload).expect_err("must reject");
            assert!(err.contains(wanted), "got: {err}");
        }
        // …and every legitimate payload still passes the gate.
        assert_eq!(
            dispatch(
                &conn,
                &json!({"operation":"select","table":"t","columns":["a"],"where":{"a":1},
                        "order_by":[{"col":"a"}],"limit":1,"distinct":true})
            )
            .unwrap()
            .error_code,
            None
    ...
    /// An unknown OPERATION keeps its own wording — the argument gate does not
    /// swallow it.
    #[test]
    fn an_unknown_operation_still_reports_unknown_op() {
        let conn = rusqlite::Connection::open_in_memory().unwrap();
        let err = dispatch(&conn, &json!({"operation":"drop_everything","table":"t"}))
