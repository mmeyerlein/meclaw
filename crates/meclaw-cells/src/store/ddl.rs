...
mod tests {
    ...
    fn fts_ddl_declares_the_stemming_tokenizer() {
        ...
        assert!(sql.contains(&declared), "got: {sql}");
    ...
    /// The versioned name IS the migration signal (W13 hardening).
    ///
    /// An index carrying a PREVIOUS tokenizer name is the case the version bump
    /// creates, and it is the sharp one: the connection has to be able to OPEN
    /// that index (FTS5 resolves the tokenizer at open time) before it can drop
    /// it. This test starts from a legacy declaration, so it fails as a spawn
    /// error the moment the legacy name stops being registered — which is
    /// exactly the trap a future `_v2` bump would otherwise walk into.
    #[test]
    fn fts_ddl_rebuilds_an_index_declaring_a_previous_tokenizer_name() {
        let conn = conn_with_extensions();
        conn.execute_batch(
            "CREATE TABLE facts (id TEXT PRIMARY KEY, claim TEXT);
             INSERT INTO facts VALUES ('1','hat lieblingseditor helix');
             CREATE VIRTUAL TABLE facts_fts USING fts5(claim, content='facts',
                 content_rowid='rowid', tokenize='meclaw_stem');
             INSERT INTO facts_fts(facts_fts) VALUES ('rebuild');",
        )
        .expect("the legacy tokenizer name must stay openable on a live connection");
        assert!(
            !index_declares_stem_tokenizer(&conn, "facts_fts").unwrap(),
            "a legacy declaration is NOT the current declaration — that is the signal"
        );
        ...
        let hit: i64 = conn
            ...
            .unwrap();
        assert_eq!(hit, 1, "the rebuilt index answers the plural question");

        // Second boot is a no-op: the declaration is now the current one.
        apply_fts_ddl(&conn, &fts_decl("facts", &["claim"]), &BTreeMap::new()).unwrap();
        let again: i64 = conn
            .query_row(
                "SELECT count(*) FROM facts_fts WHERE facts_fts MATCH '\"lieblingseditoren\"*'",
