...
pub const TOKENIZER_NAME: &str = "meclaw_stem";

/// The same name for the C API.
const TOKENIZER_NAME_C: &CStr = c"meclaw_stem";
...
/// Register the stemming tokenizer on a `store` `cell.db` connection.
///
...
pub fn register(conn: &rusqlite::Connection) -> rusqlite::Result<()> {
    ...
    unsafe {
        ...
        let rc = create_tokenizer(
            api,
            TOKENIZER_NAME_C.as_ptr(),
            api.cast::<c_void>(),
            &raw mut module,
            None,
        );
        if rc != ffi::SQLITE_OK {
            return Err(rusqlite::Error::SqliteFailure(
                ffi::Error::new(rc),
                Some(format!("could not register tokenizer {TOKENIZER_NAME}")),
            ));
        }
    }
    Ok(())
