...
Releases up to and including 0.28.0, from the first public release in May 2026
to the end of August. The living changelog is [CHANGELOG.md](CHANGELOG.md).

These entries are kept as they were written. Nothing here is edited after the
fact.
...
- **`member`: the recall doors promote `memory_call_id` into context**
  ...
  leg, and the brain saw "tool result lost". `member@1.1.0`; the versioned
  pins pulled `org@1.1.0` and `meclaw-os@1.0.3` behind it, each stage reported
  by its own test.
...
- **The four composition levels ship as templates**
  ...
  | `assistant@1.0.0` | the reasoning core (`cogny@4.0.2`) and the tool surface (`tools@1.1.0`) | 6 / 7 | 21 | 0 |
...
- **`tools@1.1.0` — the tool surface of one assistant as ONE node with ONE
