...
[![tests](https://img.shields.io/badge/tests-2598%20passing-brightgreen)](#)
...
meclaw is **v0.3.0**. A proof of concept for the DSL and the self-modifying substrate, with a schema that's deliberately frozen.

Real and tested today: the full actor substrate, all 13 built-in cell types, hot and cold lifecycle, runtime mutations, the template system, long-running cells, the HTTP API and web UI, the builder-hive, agent harnesses as supervised child processes, and child colonies composed as single cells. **2,598 tests. 0 fail. And climbing.** The hot routing paths are byte-pinned against fixtures, so they can't quietly drift.
