...
[![tests](https://img.shields.io/badge/tests-3588%20passing-brightgreen)](#)
...
A tool-loop is `llm → dispatcher → tools → collector → llm`, with the loopback condition sitting on one edge. You don't switch a loop on. You compose one. And once it exists as files, the swarm can rebuild it without asking you. Both halves of that loop ship as templates under `builder/templates/`, next to `session-keeper@1`, `summarizer@1`, `retry@1`, `archive-bridge@1`, `firewall@1`, `receptionist@1`, `memory-drain@1` and the `talky@1` composite that carries four of them as sub-units — none of them a line of Rust.
...
meclaw is **v0.7.0**. A proof of concept for the DSL and the self-modifying substrate, with a schema that's deliberately frozen.

Real and tested today: the full actor substrate, all 13 built-in cell types, hot and cold lifecycle, runtime mutations, the template system, long-running cells, the HTTP API and web UI, the builder-hive, agent harnesses as supervised child processes, and child colonies composed as single cells. **3588 tests. 0 fail. And climbing.** The hot routing paths are byte-pinned against fixtures, so they can't quietly drift.
