...
| `mailbox_size` | *(optional)* Bounded-mpsc capacity; overrides the colony default (`colony.json` `mailbox_default_capacity`, default 1000). A full mailbox does not stop the colony: further messages wait in the cell's overflow and are delivered in order (GH #850, `colony.json` `mailbox_overflow_*`). See `meclaw-overview.md` § Mailbox and backpressure. |
...
*(optional, only for stateless cells)* The maximum number of concurrently running worker tasks in the stateless-cell dispatcher (see `meclaw-overview.md` § Stateless cell dispatcher). It lives in `params` and not in the `cell` block. Default: different per cell type and fixed in that type's params parser — `code` and `bash` 4, `file`, `edit` and `web_search` 8, `web_fetch` 32; on `code`, `runner_mode: "resident"` forces 1. On `code` with `runner_mode: "warm"` the same number is also the size of the pool: the cell starts at most that many children, each one on the first message that needs it, and they stay after a burst until the cell is instantiated again (GH #852; `defer-register.md` § idle reaping of warm runner children). Configurable per cell, for example `web_fetch` with `32` (HTTP provider rate limits), `file` with `8` (disk I/O), `bash` one-shot with `4` (process resource limit). For stateful and long-running cells the value is ignored.
...
A system unit with `Delegate=yes` needs no `DelegateSubgroup=` (GH #686). The init system hands the unit's directory over with `cgroup.controllers` filled and `cgroup.subtree_control` empty — enabling controllers is the delegatee's step under cgroup v2, and the daemon takes it itself: it enables the controllers it needs inside the delegated directory, and when it is the only process there it first moves itself into a sub-group (`meclaw-daemon`) below, because the kernel enables controllers only in a directory without processes. So a daemon under a transient user (`DynamicUser=yes`), for which no `user@<uid>.service` exists, finds its cgroup too. Measured with `--sandbox-probe`: `limits yes` under `DynamicUser=yes` + `Delegate=yes`, with and without `DelegateSubgroup=`. What still forces `limits no` is `ProtectControlGroups=yes`: it mounts `/sys/fs/cgroup` read-only, and a delegatee that may write nothing can enable nothing — the directive stays off in a unit that wants caps.
...
A cell type may go one step further and require a secret to be a token. The `meclaw` platform of `proxy` does (GH #828): `auth.value` and `auth.client_secret` must stand in `config.json` as exactly one `${VAR}` without a default. A literal would put the credential on disk and into every export of the tree, and `${VAR:-fallback}` would do the same as soon as the variable is unset. The check needs the file as it is declared, because the resolved value looks the same wherever it came from, so the substrate hands each cell type its `params` twice: resolved to `validate_params`, declared to `validate_declared_params`, at boot and at the mutation door alike. The refusal names the key and never the literal. In the cell's `contract.settings` the block is marked `"secret": true`.
