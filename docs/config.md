...
- **`cell` block = colony substrate.** These fields control **how the colony** instantiates, registers, and supervises the cell. They are **never** handed to the cell. The cell sees only its `params` block plus the message it is currently processing. Allowed keys: `id`, `type`, `timeout`, `restart_limit`, `idle_timeout_ms`, `mailbox_size`, `message_timeout`, `provenance`, `surface` (details in the `cell` table below) plus `template` — **template-time only**, exclusively in a `ref` marker (§ Special case template reference): the subtree parser reads it, the boot does not, and it never stands in an instantiated `config.json` because the reference is already resolved by then. **Correction (GH #277):** this said "A key declared in the `cell` block that is not allowed is a boot error." That is false, and always was — `CellHeader` (`crates/meclaw-colony/src/config.rs`) carries **no** `#[serde(deny_unknown_fields)]`, and no other check enforces the list either: an unknown key in the `cell` block is **silently ignored** today and the boot goes through. The list above is the documented convention this tree keeps to, not an enforced gate. A typo in a `cell` key therefore passes unnoticed and behaves like an option that was never set. The enforcement is an open gap; until it is built, read the list as a style rule, not as a barrier.
...
| `type` | Cell type (`hive`, `store`, `llm`, `bash`, `code`, `web_fetch`, `web_search`, `file`, `edit`, `proxy`, `timer`, `mcp`, `harness`, `subcolony`). Together with `id`, the **immutable** part of the `cell` block. Plus `ref` — **template-time only** (see § Special case template reference above): it is resolved at instantiation and stands in no instantiated `config.json`. |
