...
| `timeout` | Hot/cold mode (see `meclaw-overview.md` section "Hot/cold cell model"): `0` = default (idle-timeout model, Awake↔Asleep), `>0` = one-shot (despawn after each message), `-1` = persistent (typically `proxy`/`timer`/`mcp`/`web`/`voice`, never despawn). Phase-13 activation; before that, all cells are permanently a task. |
...
| `…[].required` | `true`: the instantiating mutation **must** wire this lane — onto the hive path for a rim lane, onto one of the `at` connect points for a lane that docks below the rim. Without that edge the **birth** is refused with `hive_contract`. Checked once, at birth; a later `remove_edges` on a standing hive is not judged. Only meaningful on `accepts`; on `emits` the key is meaningless (nobody wires an exit from outside). Optional, default `false` (apps rim, 2026-09-05). |
