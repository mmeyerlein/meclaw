...
**Live-read at every boot.** `config.json` is re-read and re-parsed on every boot, and `${VAR}` tokens are substituted in memory against `{root}/.env` each time. The on-disk file is never rewritten at boot — instantiation is its only writer. `.env` is therefore live: changing a value and rebooting changes the effective params. The escaped form `$${VAR}` is written to disk as a literal `${VAR}` at instantiation and is consequently the only form that re-binds per environment; a plain `${VAR}` is resolved once, at instantiation, and its resolved value — secrets included — is what an exported tree contains.
