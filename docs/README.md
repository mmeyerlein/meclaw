...
| speak to a colony, or write a client that does | [`voice-wire-protocol.md`](voice-wire-protocol.md) | The `meclaw-voice/1` frames the `voice` cell speaks: the query parameters, the audio format, every text frame in both directions, the close codes and a worked example session. The cell type itself is in [`cell-types.md`](cell-types.md) § `voice`. |
