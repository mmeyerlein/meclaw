...
2. The message in flight is rejected with `error_code: "credential_pending"` — **no** provider was called and nothing was billed. The cell does **not** put it in a queue: a queue with a timeout and a re-delivery would be a second state machine inside a cell that has none today (concurrency invariant: one task, one state, no lock). The next message goes through normally.
...
| `error_code` | only on `finish_reason == "error"`: `"rate_limit"` \| `"auth"` \| `"timeout"` \| `"model_not_found"` \| `"provider_error"` \| `"invalid_input"` (W4b: param-update reject, immutable/unknown/malformed key) \| `"credential_pending"` (R3/GH #421: the cell declares `params.credential_grant_id` but holds no credential in RAM yet; it asked the access hive for one in the same turn and answers the next message normally — no provider was called and nothing was billed) |
