...
- `{"operation": "import", "from": "<dir>", …}` → reads the same directory back, one transaction per table. Without `table`/`tables`, every `<dir>/seed/*.jsonl` in name order. Receipt `{format, seed_dir, tables[{table, rows_in_part, rows_written, rows_skipped}], rows_written, rows_skipped}`.
