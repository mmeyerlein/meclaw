...
**The owner of `attachments[]` resolution is the consuming cell** (ruling GH #19). The substrate resolves, at the delivery boundary, the pointers whose target is a **body document** (`messages_id`/`text_id`); an attachment is not one, it is a file of arbitrary type and arbitrary size. Inlining it into the JSON body would defeat the very blob store it came from: a 40 MB PDF does not belong in a message. The `blob_id` ref therefore reaches the cell **unchanged**, and the cell reads the blob **on demand**, at `handle()` time, via the storage abstraction. Attachment processing thereby stays a cell capability and is not a substrate detail. (The wiring, handing a cell a store reader, arises with the first real consumer; today the slot has none, see `docs/roadmap.md`.)
...
**Two other pointer classes, explicitly not here:** the `{text_id}` leaves in the `system` tree (a different site with a different target shape, a leaf becomes `{"text": …}`, not a turn) and the `blob_id` refs in `attachments[]` (see § `attachments[]` schema, the owner is the consuming cell). Both travel through the delivery boundary unchanged.
