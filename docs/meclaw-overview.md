...
4. Cell inits run asynchronously. On init failure the cell restarts `one_for_one` (5 retries by
   ...
   `{"mutation":{"id":…,"outcome":"committed","changes":[…]}}` on success, `"outcome":"rejected"` plus
...
5. The door leaves a receipt (GH #553, ruling R-0904-1). The verdict in step 4 reaches only whoever
   ...
   `outcome`, `scope`, `form`, `changes`. `changes` names every child of a replaced node: kept,
   replaced, added or left, with the version it came from and the one it went to, and is `[]` for
   every other knock (GH #682); a replaced entry also carries `parked_path` and `parked_store`, where
   the old child was parked and whether a `cell.db` went with it (GH #773); every entry names the
   `templates` rows it stood on and stands on as `from_template_id` / `to_template_id`, absent where
   there is none (added / left, or a stamp the library has no row for, GH #811); the single form lists them sorted by path, the manifest form one
   sorted block per entry in manifest order. It knows no recipient; the hive's edges fan it out.
   Without the key nothing changes. **And the boot is the first one** (ruling O-0904-2): `form: "boot"`, nil id,
...
```json
{
  ...
  "diff": {
    ...
    "replace_nodes":[ { "match": { "name": "..." }, "with": { "template": "...", "params": {} } } ],
...
The `diff` takes exactly these nine keys. A key no operation reads, whether a typo (`add_node`), a
...
```json
{ "manifest": { "outcome": "committed", "applied": 5, "ids": ["…","…","…","…","…"], "changes": […] } }
```
...
| `swap_nodes` | **Graph swap**: swings all external edges of an implementation (`match`) atomically onto another (`with`), the other being either freshly instantiated from a template or an already existing cell. The old cell remains disconnected and preserved (no-delete; swappable back at any time by swinging the edges back). `swap_nodes` is a pure edge and topology diff: no `config.json` rewrite of an existing cell, no `cell.db` migration, no `cell_id` takeover (the new implementation has its own identity), and it inherits the atomicity model of the edge mutation. What "external" means for a subtree (GH #256): an edge is external when its other endpoint lies outside the subtree rooted at `match`. The wiring with which that root serves its own children (`<unit>` to `<unit>/<cell>` and back) is internal and is not carried along; it stays with the unit it belongs to. The old unit is thereby preserved whole, which is what makes swinging the edges back restore a working unit. On a leaf the difference is invisible, which is why it went unnoticed until GH #256. Conditions for the instantiate form: the `with` target path is free in the registry and on the filesystem, and the template named is not a single hive cell (`hive_template_single_cell`, GH #572; a hive has no factory and enters the world only as the root of a multi-cell subtree, and a subtree template is refused at this door with `schema`: for a standing node it goes through `replace_nodes`, a new one is grown by `add_nodes`). A directory already lying there that no registry row names (a hand-placed tree, the residue of an aborted migration) is refused by name rather than overwritten; taking it over is done with an `add_nodes` at the same path (a resume) or an `add_nodes[].adopt` stating the `cell.type` expected there. `with.params` runs through the same override-key check as `add_nodes[].override_params` (GH #666): a key the named template does not declare is refused pre-destructively with `schema`, in the flat form, because the instantiate form takes a single-cell template. It asks the `operator_set` question too (GH #661): a param the named template declares as operator-set has to be in `with.params`, otherwise `operator_param_unset` — the door that grows a node is the same door whichever operation reaches it. |
| `replace_nodes` | **In-place lift** (GH #682, ADR-0040): a standing node is lifted to a new version of its template in place. `{"match": {"name": "display"}, "with": {"template": "screen@1.1.0", "params": {…}}}`: `match.name` resolves against the registry and the hive scopes like `swap_nodes[].match`, `with.template` is the version to lift to, `with.params` the override form of that template (as `add_nodes[].override_params`, path-keyed on a subtree, key check GH #666), an `operator_set` param the target version declares has to be in `with.params`, otherwise `operator_param_unset` (GH #661), and there is no `with.name`: the node keeps its path, so every outer edge keeps its endpoint and is not touched. The operation compares the new template child by child with what stands and gives each child one of four verdicts. **kept**: its `cell` (less `cell.id` and `cell.provenance`), `params` and `contract` are the same JSON the template would render (ref defaults and the lift's `with.params` laid on `params`, a `${ctx.*}`/`${uuid7:*}` token compared on its literal segments), and that holds for a `ref` child too, whose moved reference hop alone condemns nothing (the kept child keeps its old stamp); `description` documents the template version the provenance stamp names and takes no part in the verdict (GH #773); a child without a provenance stamp counts as changed. A kept child is not written, not seeded, not restarted: `cell.id`, `cell.db` and its registry row stay. **replaced**: the new child is instantiated under the child's own name, and the old directory with its registry rows (whole subtree) is parked beside it as `<name>~<old-version>`; a second lift from the same version parks as `<name>~<old-version>~2`, and so on; the receipt names the park path (`parked_path`) and whether a `cell.db` went with it (`parked_store`, GH #773). **added**: a child the old version did not have is instantiated like a `missing` node of a subtree merge. **left**: a child the new version no longer names stays where it is, no inner edge names it any more, the recompute stops it (no-delete) — unless a deep outer edge still points at it, in which case it stays alive. Every parked `~` sibling of an earlier lift is a node the template does not name either, so it is reported as `left` again on every later lift. A parked node takes no edges (`add_edges` endpoint, subtree-internal edge, `swap_nodes[].with.name`: `schema`) and is not lifted (`replace_nodes[].match.name`: `schema`, the live node beside it is the one to lift), and `~` is reserved for it: no name an operator writes at `add_nodes[].name`, the instantiate form of `swap_nodes[].with.name` or `move_nodes[].to` may carry the character (`schema`). The hive's own `config.json` is written anew from the template, with the same `cell.id`; its lanes and inner edges are the new template's: the inner edges are replaced INSERT-BEFORE-REMOVE (an old edge equal to a new one, lane included, keeps its id, so a kept child is never unwired for a moment), the outer edges stay untouched, and the renewed contract holds from the next mutation on. Lanes may be added freely; a lane the new version drops while an outer edge still uses it is refused pre-destructively with `hive_contract`. One node is lifted once per diff (a second entry on the same node, in whichever spelling, is a `naming_collision`), a hive is not lifted to a leaf template nor a leaf to a hive template (`schema`), and a leaf template on a leaf is the same act in small: the old cell is parked beside the new one as `<name>~<old-version>`, the edges stay. Everything before the first rename is pre-destructive (`template_missing`, `match_no_hit`, `schema`, `hive_contract`, `naming_collision` when the chosen park name already has a registry row); every refusal after the moves, the stop-wiring guard among them, renames the old children back, takes the fresh directories out, restores the declaration and moves the rows back. Then one recompute over the subtree: the parked and the left children lose their edges and are peace-stopped, the new ones are spawned. The committed receipt says what happened to every child (§ The mutation flow, `changes`). The way back is the same operation with the old version. |
...
- `template_version_immutable` (GH #811): an `add_templates[]` entry declares a `name@version` the
  colony already stores, and its files differ from the stored ones. A stored version is immutable:
  nodes stand on it and the way back of a lift names it, so a changed tree has to arrive as a new
  version, which registers beside the old one. Decided against the staged bytes, before anything
  moves into the library; the identical files stay `template_name_taken`. Emitted by
  `mutation::register::stage_registrations`.
...
| `/colony/templates` | Read the templates registry (for builder discovery); every version a row exists for, each with `template_id`, `name`, `version`, `filesystem_path`, `author` and `scanned_at` (GH #811); with `?name=` also `used_by`, the registry paths whose provenance chain names that version — the chain joined to the table, no new column | `?type=<celltype>` (exact match on the template cell type; unknown values yield an empty list), `?name=<name>` | no | 5 |
...
| `/colony/graph` | Read the topology of a scope (nodes and edges, runtime-projected); a node carries `path`, `cell_type`, `active` and `parked` (a lift's parked predecessor, GH #773) | `?scope=<path>` (default root), `?tag=<token>` | no | 6 |
...
| `/colony/messages` | Browse the message log: a newest-first list with filters plus a single message | `?id=<uuid>`, `?trace_id=<uuid>`, `?parent_message_id=<uuid>`, `?correlation_id=<uuid>`, `?to_path_prefix=<path>`, `?from_path_prefix=<path>`, `?body_kind=inline\|blob`, `?since=<ts>`, `?until=<ts>`, `?before_created_at=<ts>&before_id=<uuid>` (keyset cursor), `?limit=<N>`, `?scan_budget=<N>`, `?resolve_blob=true` (resolves at most 25 blob bodies per page; when it stopped short the answer says `blob_resolution_truncated: true` and the remaining rows keep their `body_payload` uuid) | no | P1 |
...
| `--env-report` | GH #826 | off | A question about the root's `.env`, not a colony run: which keys no `${…}` in the tree substitutes (`unused`), and which strict `${KEY}` references name a key the file does not carry (`missing`, with the cell directories that ask). Names only, never a value. Reads files and nothing else: takes no root lease, opens no `colony.db`, writes no `log.jsonl`, so it answers for a colony that is running. Exits 0 with findings; non-zero only when the `.env`, a `config.json` or the templates directory cannot be read, when the root does not hold exactly one top-level cell directory, or when a `${` has no closing `}` (named by file, JSON pointer and position, never by the string). A `.env` that does not exist reads as zero keys, and a line of it that does not start with a `KEY=` name is counted in a note and never printed. Takes precedence over `--validate`, `--api`, `--daemon` and `--apply` |
...
one binary, many flags, one mode switch (`--daemon`, `--validate`, `--sandbox-probe`, `--env-report`). Operations
...
prefix `/ui/*`. `GET /` answers `307 Location: <prefix>/ui/`, where `<prefix>` is the `X-Forwarded-Prefix` a path proxy sent, read with the same grammar the web cell uses, or empty. It is server-rendered HTML via `maud`, with no
...
|---|---|---|
| `config.json` (cell) | Colony | **only** on instantiation (template copy, UUID assignment, `${VAR}` substitution); the `swap_nodes` graph swap does not rewrite an existing `config.json`. The one exception is a hive's declaration: `replace_nodes` writes the hive's own `config.json` anew from the template; a hive is a scope, not a cell, and its declaration is what the operation replaces (GH #682) |
...
no `cell.db`. Under a lift (`replace_nodes`) a replaced child starts with a fresh `cell.db`, a kept
child keeps its own; the replaced child's `cell.db` is parked with its directory and named in the receipt
(`parked_store`, GH #773). The wipe path is deferred: emptying a `cell.db` is an operator action outside
the mutation flow, and the mutation vocabulary has no verb for it.
...
The builder is the installer (GH #599): an app's `template.json` carries its wiring as a
declaration under `app`, the wish hands that block to the fast-lane recipe `install_app`, and the
recipe renders the mutation (`templates/builder/README.md` § An app is a declaration). One wish and
the declaration it renders stand with their placeholders in `templates/member/README.md`
§ Installing an app.

What is not here: the app loader is a later errand (register `reg:app-loader`). There is no
package manifest beyond that declaration, no `app.json` read at start, no colony-level app listing
and no `app install` command.
...
(GH #664; until then the name alone was unique, GH #277) — unless one of the two is the colony's
kept copy `local/<name>@<version>/`, which then wins (GH #811, see Scan strategy). Two different versions of one name are two
...
- **The colony keeps every version it instantiated (GH #811).** After a committed `add_nodes`,
  `swap_nodes` or `replace_nodes`, every `name@version` the mutation stamped onto a node — its own
  template and every hop of its `template_chain` — is copied to `local/<name>@<version>/` (the same
  path a versioned `add_templates` builds), and the row is repointed there under the same
  `template_id`. The same happens at boot, which is the migration of a colony that instantiated
  before, and before every rescan, so that a rescan after a library swap cannot drop a row before
  its bytes are safe. Unversioned templates are not kept: no pinned reference could name the copy.
  A shipped library is therefore free to move on — replacing `templates/<name>/` with the next
  version leaves the older version registered and instantiable, and the way back of a lift needs no
  `add_templates`. A tool that rebuilds `templates/` must leave `local/` alone.
- **`local/` is the colony's own.** The kept copies and every `add_templates` registration go to
  one directory the colony owns: `{templates_root}/local` when the library lies under the colony
  root (`<root>/templates`, or a `--templates` inside it), `<root>/templates/local` when it does not.
  A library outside the colony root — a repository checkout, a library several colonies share — is
  never written; the scan reads the colony's directory as a second root beside it, the kept-copy
  rule below holds across both, and rows in that directory do not make the index foreign to the
  library (GH #61).
- **The kept copy wins.** When the scan meets a `(name, version)` twice and exactly one of the two
  is the kept copy `local/<name>@<version>/`, the kept copy is the entry: a byte-identical twin
  falls away silently, a twin with different bytes is skipped with its reason
  (`template_version_immutable`, naming both directories) instead of aborting the scan. Two
  duplicates of which neither is the kept copy still abort.
- The scan is recursive with one exclusion: a directory `.keep-*` is a copy the colony is still
  assembling (GH #811) and is not read. `scan_templates_dir`
...
without a directory. Since GH #811 that reaches only rows without a kept copy: a row the colony
instantiated from points at `local/<name>@<version>/`, which no library swap touches. Existing
instances keep running, because they have their own filesystem copy.
...
(`<local>/<name>@<version>/` in the colony's own `local/`, see Scan strategy, or `<local>/<name>/`
when it declares no version) as a
...
- The strict variant `${VAR:?error_msg}` is not supported; any other `${VAR<op>...}` form besides
  ...
  pass-through.
- `meclaw --env-report` (GH #826) compares the file with the tree: it names every key no `${…}` in
  any `config.json` substitutes (the whole file counts, `script_inline` and defaults included, and a
  `ref` marker counts the template it grows) and every strict `${KEY}` whose key the file lacks.
  It prints names, never a value, and reads only the `.env`: a cell's `env_passthrough` and the vault
  unlock read the process environment, which the report does not look at.
