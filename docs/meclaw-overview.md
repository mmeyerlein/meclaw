...
**Reply body form (reads):** colony answers in the universal body format with a top-level slot named after the endpoint: `registry`, `dead_letters`, `templates`, `trace`, `mutations` (audit read), `rescan` (outcome). Analogous to the `graph` slot (see "Visibility").
...
**`?limit=<N>` defaults** (for `dead_letters`, `trace`, `mutations` audit read): default **100**, hard cap **1000**, **no config knob** (no speculative spec surface). The cap also brakes the routing-loop stall of DB-heavy reads.
...
| `/ui/dead_letters` | List of the most recent dead letters with `error_code`, path, body preview | `/colony/dead_letters` |
