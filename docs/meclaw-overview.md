...
5. **The door leaves a receipt** (GH #553, ruling R-0904-1). The verdict in step 4 is an *answer*: it reaches only whoever set `reply_to`, and neither `POST /colony/mutations` nor `meclaw --apply` sets one. Anything else that wanted to know the graph had moved had to ask on a timer — a poll in an event-driven substrate, paid for in availability. So when `colony.json` carries the `mutation_receipts` key, the door additionally emits **one** terminal event to the hive named there: `hop.route == "mutation_committed"` plus `mutation_id`/`mutation_ids`, `outcome`, `scope`, `form`. It is not an answer and knows no recipient — the hive's edges fan it out. Without the key nothing changes. The first one is the **boot receipt** (`form: "boot"`, nil id) right after `InitialApply`, so a restart need not wait for the first mutation. Keys and guarantees: § `colony.json` — schema.
...
**A colony carries many organisations and exactly ONE OS, and the OS hands out what is system-near** (ADR-0022, GH #543). An organisation is a namespace: it holds no port band, assigns no port and has no configuration surface of its own — it **asks the OS** for one. System-near means: scarce, colony-wide, and two holders of one is a collision rather than a disagreement — a TCP port, a bind address, a socket. This is a statement about **who may answer**, not about a mechanism: how an organisation will ask (a lane, an endpoint, a broker capability) is open. The form it has today is the builder at the shell level (`templates/meclaw-os`), which gives every grown member the port of its screen as `screen_port_base + <the member's index in its organisation>` — the builder is part of the OS (ADR-0015), so that is the first form of this responsibility and never an organisation's own right. The index is **measured**, off `/colony/graph`, before anything is rendered: an allocator that does not look at what it has already allocated is not one.
...
**`.` names the scope root itself** (GH #487). `{"from": "./firewall", "to": "."}` is the ordinary spelling of *a lane that leaves this level*: `.` — and the `./` that means the same — resolves against the scope in which the schema is declared, by the same rule as every other relative path (`Path::resolve`, "stay at the sender", see § Path resolution: edge cases). This holds **symmetrically in both usages**, like the depth rule above it: in a bootstrap `params.graph`, `.` names the hive whose `config.json` carries the schema; in a mutation diff, the `scope` of the declaration. Until GH #487 it held in the first usage only — 277 of the 561 shipped template edges are spelled that way, and `add_edges` refused the same string as `edge_schema` ("to='.' unknown"). It is resolved at the point of **use**, never in the document: a diff keeps the spelling it was submitted in, or a manifest digest would describe a document nobody was shown (§ Mutation format). A scope whose root is **no registered node** — neither a cell nor a hive scope — stays `edge_schema`: the rule widens the vocabulary, it invents no node. The root scope `/` is therefore not a special case but the ordinary one: where a hive scope marker sits there, `.` names it like any other.
...
| State | Templates |
|---|---|
| `ports: []` + `contract` — done | `collector`, `session-keeper`, `summarizer`, `memory-drain`, `affinity` (with the `ref` sub-units inside `talky` and `cogny`; the `summarizer` is part of no composition since `talky@4.3.0`) |
...
**The `diff` takes exactly these eight keys.** A key no operation reads — a typo
...
| `add_edges` | New edges in colony's edge table, scoped. Endpoints are paths relative to the `scope`, at any depth; **`.` names the scope root itself** (GH #487, § Graph schema → `edges`) — the very spelling a `params.graph` uses for its own level. Optional fields as in the bootstrap schema: `condition`, `modifier` and — since **v0.18.0** (GH #283) — `default` (boolean, absent = `false`). `"default": true` puts the edge into the second routing phase (§ Edge model); a non-boolean value is `edge_schema`, and an unguarded default edge commits with a `warn` log line. |
...
- `seed_target_not_a_store`: a `seed_rows` entry names a target that is not a `store` — nothing stands there, a cell of another type stands there, or its declaration cannot be read. Only a `store` owns declared tables (GH #456).
- `seed_table_undeclared`: a `seed_rows` entry names a table the target does not carry in `params.schema`; the refusal names the tables that do exist. Inventing the table would give the store a shape it never agreed to (GH #456).
...
- `hive_template_single_cell` (GH #572): an instantiating diff entry names a template whose root is a hive and which has **nothing** under it. A hive is a scope marker, not an actor: it has no factory, so a single cell of that type cannot be spawned at all. A hive enters the world only as the **root of a multi-cell subtree** — the subtree door stages the whole unit and registers its hive scope. It holds at **both** instantiating doors (`add_nodes[].template` and the instantiate form of `swap_nodes[].with`), because it is one question; a template with cells under it never produces this code. Pre-destructive, emitted by `reject_if_single_cell_hive_template` (`mutation/validate.rs`) at stage 4 — before it the same entry validated clean, staged a directory and died in the apply arm as `spawn: factory missing for hive`. The refusal names the shape that works (ADR 0021).
...
**Attribution is a different question, and it has an answer.** No permission layer does not mean no record of who asked. The substrate stamps `envelope.reply_to` on every **cell** emission, so a message a cell sent carries the path it came from — which is why a submission drafted inside a colony reaches the mutation door under a name, and lands in `mutation_log` with it. A `POST` from outside has no such path: it is not a cell, so there is nothing to stamp, and a submitter reading the envelope sees an anonymous request. The template answer to that is `operator` (`templates/operator/`), a hive whose occupants turn an outside request into a message they emit themselves — the request acquires the identity of the cell that served it. That is IDENTITY and not AUTHENTICATION: the paragraph above still holds in every word, the hive verifies no claim, and what stands in front of the port is still yours.
...
| `mutation_receipts` | **GH #553, ruling R-0904-1.** Opt-in (default: the key is absent): `{"to": "<path>"}` says **where** the mutation door leaves its receipt. With it set, the colony emits exactly one terminal message per **committed** knock at `/colony/mutations` to that path — sender `/colony`, the mutation's `trace_id`, `parent_message_id` the knocking message, a fresh `message_default_ttl`, body `{"messages": []}`, and everything it says in the `hop`: `route: "mutation_committed"`, `mutation_id` (single form) resp. `mutation_ids` (manifest), `outcome: "committed"`, `scope`, `form: "single" \| "manifest" \| "boot"`. Five keys and no more — *what* changed is a question `/colony/graph` answers, and a receipt that tried to answer it too would be a second, staler truth. **It is the second place the substrate seeds a `hop` itself** — the first is the HTTP ingress with the `hop` field of `POST /messages` (GH #175, § HTTP endpoints), and for the same reason: a message that arrives at a hive path without asserting a lane matches no door. `to` is a **hive** path by intent: the receipt then enters as a hive transit and the hive's own `{"from": "."}` edges decide who hears it, so the substrate never learns a single listener's address. A refusal (including a manifest that stopped part-way) leaves **no** receipt, and none is emitted during the shutdown drain (the same reason GH #47 refuses source emissions there). The receipt is terminal (no `reply_to` of its own) and is not a mutation itself, so it can neither start a round nor produce a second receipt. **And the boot is the first one** (ruling O-0904-2): right after `InitialApply` has put the edge table up, exactly one receipt with `form: "boot"` and a nil `mutation_id` goes out, so a restart fills a menu and a screen instead of answering `503` until the first mutation of the day. It is the only one that carries **no** `parent_message_id` — it continues nothing — and therefore stands in the message log as `@external`, the booking of every source emission; a mutation's receipt carries the knocking message as its parent and stands under `/colony`. **A receipt is an event, not a guaranteed delivery:** it goes back into the colony's inbox as its own work, and when that inbox is full it is dropped with a `mutation_receipt_undeliverable` warning rather than blocking the loop. A listener therefore has to tolerate a missed receipt — it is healed by the next receipt, or at the latest by the next start's boot receipt. **Who hears it in the shipped library (GH #553, the template half):** the chain is drawn one level at a time — `meclaw-os` accepts `mutation_committed` and carries it into `./orgs`, `org` into `./members`, `member` into `./assistants` AND `./apps`, `assistant` into `./talky` and `./cogny`, and each of those into `./collector`. The lane keeps ONE name the whole way; only the collector turns it into its internal `in_menu_tick`, and `colony-view` turns it into its own `in_refresh`. Every level that declares the lane is a mandatory hop for it (ADR-0020). The two poll timers that used to ask the same questions every few minutes (`collector/menu-clock`, `colony-view/refresh`) are gone with no replacement. **Why opt-in:** a receipt is traffic in a topology that did not have it before — who hears it is the colony's decision, never the substrate's. A `to` that does not start with `/` is a hard parse error. |
...
| `/colony/ledger` | Counts and sums out of `message_log` / `dead_letters` / `mutation_log` for one time window (aggregates, no raw rows) | `?since=<ts>` (inclusive, default `now - 3600`), `?until=<ts>` (exclusive, default `now`), `?path_prefix=<path>`, `?cycle_id=<id>`, `?group_by=model\|path\|error_code`, `?tag=<token>`, `?scan_budget=<N>` | no | 0.20 |
...
**What the `ledger` sums, and along which axis** (GH #463). The endpoint sums exactly what is in the **hop header** — `message_log.headers` under `$.hop` — and nothing else. Since #463 the `llm` cell writes the **whole** usage block there: `tokens_prompt`, `tokens_completion`, `tokens_cached` (cache-read tokens, whichever spelling the provider used), `cost` (**only** when the provider reports a cost figure of its own — the substrate never computes one, because a price table in the substrate would be a second source of truth that goes stale silently) and `latency_ms`. `latency_ms` **also** stays in the body's `meta`: one number in two places is cheaper than a migration. A figure the provider did not report is an **absent** header key, not a zero — a summed zero and an unreported figure are the same digit and two different facts. Beside those the `ledger` sums `duration_ms`, which the tool cells (`file`, `bash`, `web_search`, `mcp`, `vault`) already wrote. Both `latency_ms` and `duration_ms` come with a **sample counter** (`latency_samples`, `duration_samples`): a mean is the sum divided by the samples, never by `calls` — most hops in a window carry no duration at all, and dividing by the call count silently answers a different, smaller question.

**`?group_by=`** picks a **second** grouping beside `by_model` — `model` (the default behaviour, `$.hop.model`), `path` (`to_path`, i.e. the **receiving** cell, the same direction the cycle counter means) or `error_code` (`$.hop.error_code`). `by_model` is answered either way, with or without a `group_by`; a caller that sends none gets exactly the #267 reply shape back, because `by_path` and `by_error_code` are then absent rather than empty. Every axis leaves out its NULL group: a hop without a model is not a model, a hop that did not fail is not an error code. The aggregates rule is unchanged: a group **key** is the axis the caller itself asked to group along — rows, envelopes and header contents still do not leave the endpoint.
...
`--apply <file|->` (GH #423): hands **one manifest** (§ Mutation format, "Second body form") to `/colony/mutations` right after the boot and prints the receipt. `-` reads it from stdin. The position is the argument: only after the boot does the tree stand that the manifest mutates. The printed receipt is terminal prose for the caller; the **mutation receipt** (GH #553) is a different thing and goes into the topology — a one-shot run whose receipt the immediately following shutdown still eats is healed by the next start's boot receipt.
...
**The four absolute edges a mutation may draw** are `-> /colony/graph` (GH #163),
`-> /colony/registry` (2026-08-27), `-> /colony/ledger` (GH #267) and
`-> /colony/templates` (GH #496). They address no cell but the authority's own read-only
...
be instantiated at all. The templates read is the fourth and was forced the same way:
the `builder-librarian` corpus is a **seed**, loaded once when the store's `cell.db` is
created, so a running librarian answers *what templates exist* about the library of the
moment it was born — and a class registered since is resolvable at the mutation door and
invisible to the composer (GH #496). It answers `template_id`, `name`, `version`,
`filesystem_path`, `author`: the colony's bookkeeping about its own **library**, about
classes rather than instances, and no message row and no header. `/colony/templates/rescan`
is a separate endpoint and stays out — it is an effect, not a read.
`/colony/mutations` (authority transfer), `/colony/trace` and
...
diff <(git show HEAD:crates/meclaw-colony/src/colony.rs | sed -n '/^async fn route(/,/^}$/p') \
     .github/fixtures/expected_route_body.txt
```
...
The overview table (type · task · actor kind · emission mode · phase) is canonical in [`cell-types.md` § Overview](cell-types.md#overview). The detail spec per built-in cell type is likewise in `cell-types.md`. Since which release a cell type has been live → `CHANGELOG.md`; what is deferred on it → `docs/defer-register.md`.
...
- **`modifier`** (operations object with CEL expressions as values) is the **sole header authority**: it promotes/computes `context.*` and refines `hop.*` before forwarding. Schema:
  ...
  - `set_context` is CEL-valued and thereby covers both: **promoting** a hop value to context (`"set_context": { "turn_id": "hop.turn_id" }`) AND **computing** (`"set_context": { "iter": "int(context.iter) + 1" }`). **Correction (GH #500):** this used to read "the `int()` cast is necessary because CEL deserializes a JSON integer as `uint` and `uint + int` is not defined" — that is retracted. The binding was the defect, not the language: the substrate bound a JSON number through serde, and serde serialises every non-negative integer as a `u64`. It now binds explicitly (see "Numbers in conditions and modifiers" below), the cast is an identity, and the spelling above stays valid — it is merely no longer necessary.
...
connect point by naming a template. That holds for **both** readers of the list
(GH #573): the port boundary and the swap's re-anchor verdict. The successor is
taken from the same deduplicated list — what stands keeps what it has, and what
this diff gives birth to counts, including a successor an earlier `add_nodes`
entry of the same diff contributed. And the birth contracts reach exactly one
...
**Numbers in conditions and modifiers (GH #500)**: a number out of a header compartment binds as the CEL type the CEL spec gives a JSON number — **`int`** while it fits `i64`, **`uint`** only above `i64::MAX` (where no `int` can hold it), **`double`** otherwise. So the form everybody writes first holds: `hop.http_status == 200` is true on a message carrying 200.

Until GH #500 it was not. The substrate bound the two compartments through serde, and `serde_json::Number` serialises every non-negative integer as a `u64` — the 200 arrived as a `uint`, and CEL equality across `uint`/`int` is `false` in both directions. The comparison produced no error and no advisory, only an edge that never fired; the emission dead-lettered as `no_route`, and the dead letter's own header showed the very 200 it had just been compared against. Ordering comparisons were cross-type from the start — `hop.http_status > 100` was true — and that asymmetry is what made the class expensive to find.

Two consequences for topologies written before the correction: the range form (`>= 200 && < 300`) and the `int()` cast stay valid unchanged (the cast is now an identity), while a **`u`-suffixed literal** (`== 200u`) no longer matches an int-bound number. Whoever wants unsigned semantics on purpose casts the compartment instead of suffixing the literal: `uint(hop.k) == 200u`. No shipped template and no example spells such a literal, and a sweep keeps it that way (`gh500_a_numeric_condition_fires.rs`).
...
- The loop counter (`iter`) lives as **`context`** (persistent over the iterations) and is incremented on the **loopback edge** via `modifier.set_context: { "iter": "int(context.iter) + 1" }` (the `int()` cast was necessary until GH #500 and is an identity since — the spelling stays valid, see § Edge expression language, "Numbers in conditions and modifiers"). Cells never touch `iter`, incrementing is edge authority. In a store-backed loop the same loopback edge also carries `modifier.restore_ttl` (GH #82): it restores the routing budget per round, and that is exactly why it is coupled to the iteration condition: the iteration is the bound, not the TTL.
...
- **Adding one at runtime: `add_templates`** (GH #440). A template enters the **instance-local** library (`{templates_root}/local/<name>/`) as a mutation declaration and is resolvable from that same mutation onwards. The **shipped** library is out of reach from there: the target path is built, not taken. It becomes **visible** only when the mutation commits (GH #443): the files land in the mutation's own staging area, a later entry of the same diff resolves against them there, and the `rename(2)` into the library runs immediately before the commit flush — so a refusal further down the same mutation leaves the library exactly as it found it, and a retry needs no manual cleanup. Details in § Mutation operations.
...
- **A third writer: `seed_rows` at the mutation door** (GH #456). The birth seed reaches only a fresh `cell.db`; the import slot reaches only a cell that takes the message. `seed_rows` is the third way and the only one that goes through the **authority's door**: a diff operation, and therefore with a digest, a gate, an access verdict and a `mutation_log` row (§ Mutation operations). It is the **same** mechanic — the same JSON→SQL binding, the table built from the declared column list, `ensure_keyed_table` repairing a missing key at the next wake — only the moment differs: the colony is running, and the target may be awake. It is meant for the class of rows that are permissions and keys, not for bulk data; bulk data keeps going through the import slot. **Unlike a seed file, a `seed_rows` IS substituted**: it is part of the diff and runs through the same `${…}` resolution as every other operation (§ Variable substitution). The boundary below holds exactly where it is written — it is about the *file*, not about the mechanism.
...
born that way carries a **durable marker** (`colony.db` `registry.dormant`, GH #491):
neither the recompute of **its own birth mutation** nor any later one makes it active
again; it is woken solely by a mutation that **addresses it itself** (§ Reconnect).
Otherwise they are brought to their edge-derived state by the first recompute that touches their
...
message of its own for it — the next mutation that **addresses it itself** wakes it.

**Reaching is not addressing (GH #491).** A node born inactive is fully wired — the
declaration marks the registry row, it does not withhold the edges. A recompute whose
scope merely **brushes** it therefore derived it active again, and `affected_scope`
expands every involved path to its **whole subtree**: a single `add_edges` declaration one
level up sufficed to wake a connector grown asleep that nobody had armed. A
**declaration beats a recompute side effect**: the birth sets a durable marker
(`registry.dormant`, schema v8) that every recompute honours. It falls exactly when a
mutation **addresses the node itself** — its path is in the `involved` set, as an endpoint
of an `add_edges`, say, or as the target of a `swap_nodes` — and from then on it is an
ordinary node and the edge table alone decides. A mutation elsewhere in the tree never
wakes it, however far its recompute reaches. **A node put to sleep with `remove_edges`
carries no marker and needs none**: it has no edge, so no recompute can derive it active,
and the only thing that can reconnect it is an edge naming it — the same address test,
arrived at from the other side.

The reconnect itself is unchanged:
...
**Derived activity is written down (GH #495).** Activity is derived, but it is not
re-derived on every boot: the boot recompute runs only for nodes **without** a registry
row, and a rehydrated node takes its activity from the persisted `status` column
(§ Persistence). Hence the rule: **a registration writes down the activity it registers,
and a recompute writes down every flip** — both halves, always. Before, only the second one
wrote, plus the declaration: an **island** nobody had declared was derived inactive
correctly and recorded as inactive nowhere (the INSERT row says `active`, and a recompute
that agrees with the registration produces no flip), so the **second boot** read the stale
row and started the whole island. This holds for the boot as for a mutation (single cell as
well as subtree). Unchanged by it: the `failed` status, which `inactive` never overwrites;
the grace of an edge-less cell, which is registered active and therefore touches none of
these paths; and the `dormant` marker, which only a **declared** birth receives — a derived
inactivity is asleep for the ordinary reason, its edges, and any recompute that finds it
connected may wake it (ADR-0018).
...
**Every `/colony/*` says its name (GH #439, GH #571).** A mutation declares itself under its id and its scope (`mutation <id> scope=<scope>`), a read under its endpoint (`colony-read /colony/graph`) — both through the same work pulse, both on the same `work_item_budget`. A read that takes long is therefore a **named** work item (`slow_work_item`) instead of nameless silence (`colony_loop`), and the trip line names the endpoint the loop was inside.
...
| What did the colony cost / produce in errors in the last window? | `/colony/ledger` | `?since=`, `?until=`, `?group_by=model\|path\|error_code`, `?path_prefix=`, `?cycle_id=` |
...
- `colony.db`: the central database with the registry (path → cell ID + status + template + dormancy marker, GH #491), the templates registry, the mutation log, the central message log, and the edge table. Colony writes; cells do not read directly.
- **The `status` column is the activity the next boot reads** (GH #495). The boot recompute runs only for nodes without a registry row; a rehydrated node takes its activity from this column. So **every registration** writes down the activity it registers, and **every recompute** writes down every flip — otherwise a derived inactivity (an **island**) does not survive the process that computed it, and the second boot starts it (ADR-0018). `failed` is a third value of the same column and is never overwritten by `inactive`; the dormancy marker beside it answers the orthogonal question of whether the sleep was **declared** (ADR-0017).
...
- The mutation format is a **diff** with eight optional operations: `add_nodes`, `add_edges`, `remove_nodes`, `remove_edges`, `swap_nodes`, `move_nodes`, `add_templates`, `seed_rows` (see the section "Mutation format" above), plus a `scope` field (the path prefix for the mutation) and a `ctx` block (for `${ctx.*}` substitution). A ninth, unknown key is not a no-op but `schema`.
...
- **EDA: a success ack to the builder.** `/colony/mutations` answers every mutation via `build_mutation_reply` (`crates/meclaw-colony/src/colony_dispatch.rs`) to `reply_to`: on success `{"mutation":{"id":…,"outcome":"committed"}}`, on rejection analogously with `"outcome":"rejected"` plus `error_code`/`details`. Without a set `reply_to` only the logging remains — **and the receipt** (GH #553): when `colony.json` carries the `mutation_receipts` key, the door additionally leaves every committed knock as a terminal event (`hop.route == "mutation_committed"`) at the hive named there. That is how `POST /colony/mutations` and `meclaw --apply`, neither of which sets a `reply_to`, become observable at all. Two-phase builders (mutation out, verdict back, receipt from it) build on the answer.
...
**Sub-phases** (emergent substrate intermediate passes like 6.5 / 7.5 or doc consolidations like 9.5) are not a roadmap component. What of this roadmap has shipped is recorded in `CHANGELOG.md`; what was deferred, in `docs/defer-register.md`. The phase history up to 2026-08-14 is archived in the maintainers' private tree and does not travel with this clone.
