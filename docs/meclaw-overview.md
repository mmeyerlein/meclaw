...
5. The door leaves a receipt (GH #553, ruling R-0904-1). The verdict in step 4 reaches only whoever
   ...
   every other knock (GH #682); the single form lists them sorted by path, the manifest form one
...
child keeps its own. The wipe path is deferred: emptying a `cell.db` is an operator action outside
